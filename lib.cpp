...
std::string GetClipboard(std::vector<int> closeAfterFork) {
  ...
  if (IsWayland(closeAfterFork)) {
    getClipboardCommand = {"wl-paste", "-t", "text/plain", "-n", }; // text/plain needed for long json inputs
  }
  wsl = IsOnWSL();
  if (wsl) {
...
bool SetClipboard(std::string clipboard, std::vector<int> closeAfterFork) {
  ...
  if (IsWayland(closeAfterFork)) {
    setClipboardCommand = {"wl-copy", "-t", "text/plain", }; // text/plain needed for long json inputs
  }
  if (IsOnWSL()) {
